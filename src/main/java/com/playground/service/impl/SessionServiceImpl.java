...
public class SessionServiceImpl implements SessionService {
	...
	public boolean storeSession(StoreSessionDTO data) {
		...
		if (!sessionStream.isPresent()) {
			...
			webClientBuilder.build().post().uri("52.42.41.198:8082/v1/user/session/store/airtable").body(Mono.just(session), Session.class).retrieve().bodyToMono(Object.class).block();
		...
		try {
			...
			if (result.getStatusCodeValue()==200) {
				...
				if (recordingStream.isPresent()) {
					...
					webClientBuilder.build().post().uri("52.42.41.198:8082/v1/user/session/recording/airtable").body(Mono.just(recordings), Recordings.class).retrieve().bodyToMono(Object.class).block();
				}
			}
		} catch (Exception e) {
			log.error(e.getLocalizedMessage());
